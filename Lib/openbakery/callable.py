"""
OpenBakery callable is the wrapper for your custom check code.
...
class OpenBakeryCondition(OpenBakeryCallable):
    def __init__(
        self,
        func,
        # id,
...
class OpenBakeryCheck(OpenBakeryCallable):
    def __init__(
        self,
        checkfunc,
        ...
        suggested_profile=None,  # A suggestion of which openbakery profile
...
def condition(*args, **kwds):
    """Check wrapper, a factory for OpenBakeryCondition

    Requires all arguments of OpenBakeryCondition but not `func`
    which is passed via the decorator syntax.
    """
    if len(args) == 1 and len(kwds) == 0 and callable(args[0]):
        # used as `@decorator`
        func = args[0]
        return OpenBakeryCondition(func)
    else:
        # used as `@decorator()` maybe with args
        def wrapper(func):
            return OpenBakeryCondition(func, *args, **kwds)

    return wrapper


def check(*args, **kwds):
    """Check wrapper, a factory for OpenBakeryCheck

    Requires all arguments of OpenBakeryCheck but not `checkfunc`
    which is passed via the decorator syntax.
    """

    def wrapper(checkfunc):
        return OpenBakeryCheck(checkfunc, *args, **kwds)
