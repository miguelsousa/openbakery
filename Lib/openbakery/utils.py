...
# TODO: this should be part of OpenBakeryCheck and check.conditions
...
def name_entry_id(name):
    from openbakery.constants import NameID, PlatformID
...
class PointsPen(BasePen):
    ...
    def highestPoint(self):
        ...
        for p in self.points:
            if (
                highest is None or p[1] > highest[1]
    ...
    def lowestPoint(self):
        ...
        for p in self.points:
            if (
                lowest is None or p[1] < lowest[1]
            ):  # pylint: disable=unsubscriptable-object
                lowest = p
        return lowest
