...
from openbakery.status import DEBUG
...
def get_module_profile(module, name=None):
    ...
    try:
        ...
        # e.g. module.__name__ == "openbakery.profiles.cmap"
...
class Profile:
    ...
    def register_check(self, section=None, *args, **kwds):
        ...
        @check(id='com.example.openbakery/check/0')
        def my_check():
          yield PASS, 'example'
    ...
    def auto_register(self, symbol_table, filter_func=None, profile_imports=None):
        ...
        if it is present. If they an item is an instance of OpenBakeryCheck,
        OpenBakeryCondition or OpenBakeryExpectedValue and register it in
        the default section.
        ...
        namespace_types = (OpenBakeryCondition, OpenBakeryExpectedValue)
        namespace_items = []
        ...
        for item in all_items:
            ...
            elif isinstance(item, OpenBakeryCheck):
...
def _check_log_override(overrides, status, message):
    ...
    from openbakery.message import KEEP_ORIGINAL_STATUS, KEEP_ORIGINAL_MESSAGE
...
def check_log_override(check, new_id, overrides, reason=None):
    """Returns a new OpenBakeryCheck that is decorating (wrapping) check,
    but with overrides applied to returned statuses when they match.

    The new OpenBakeryCheck is always a generator check, even if the old
    ...
    check: the OpenBakeryCheck to be decorated
