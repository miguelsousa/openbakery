...
from openbakery.checkrunner import (INFO,
                                    WARN,
                                    ERROR,
                                    STARTCHECK,
                                    SKIP,
...
# Similar to DashbordWorkerReporter of OpenBakery Dashboard.
class WorkerToQueueReporter(OpenBakeryReporter):
    ...
    def _register(self, event):
        ...
        if status >= DEBUG:
            ...
            # turn everything in a OpenBakery/Message like object
            ...
            if hasattr(message, 'traceback'):
                # message is likely a OpenBakeryError if this is not None
                log['traceback'] = message.traceback
            if isinstance(message, Message):
                # Ducktyping could be a valid option here.
                # in that case, a OpenBakeryError could also provide a `code` attribute
