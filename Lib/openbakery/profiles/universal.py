...
def com_google_fonts_check_openbakery_version(font, config):
    ...
    try:
        response = requests.get('https://pypi.org/pypi/openbakery/json', timeout=config.get("timeout"))
    ...
    if not is_up_to_date(installed, latest):
        return FAIL, Message("outdated-openbakery",
...
def com_google_fonts_check_contour_count(ttFont, config):
    ...
    from openbakery.constants import (PlatformID,
                                      WindowsEncodingID)
    from openbakery.utils import (bullet_list,
                                  get_font_glyph_data,
                                  pretty_print_list)
...
def com_google_fonts_check_math_signs_width(ttFont):
    ...
    if most_common_width and \
       len(glyphs_by_width.keys()) > 1:
        outliers_summary = []
