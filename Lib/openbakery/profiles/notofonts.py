...
from openbakery.profiles.googlefonts import GOOGLEFONTS_PROFILE_CHECKS
from openbakery.section import Section
from openbakery.status import WARN, PASS, FAIL, SKIP #, INFO, ERROR
...
NOTOFONTS_PROFILE_CHECKS = list(filter(lambda c: c not in SKIPPED_CHECKS, NOTOFONTS_PROFILE_CHECKS))
...
def com_google_fonts_check_unicode_range_bits(ttFont, unicoderange, preferred_cmap):
    ...
    from openbakery.constants import UNICODERANGE_DATA
    from openbakery.utils import (compute_unicoderange_bits,
...
def com_google_fonts_check_noto_manufacturer(ttFont):
    ...
    if not designer_urls:
        bad = True
        yield WARN,\
              Message("no-designer-urls",
                      "The font contained no designer URL"
                     )
...
def com_google_fonts_check_noto_designer(ttFont):
    ...
    if not designers:
        ...
        yield FAIL,\
                  Message("no-designer",
                          "The font contained no designer name"
                         )
...
def com_google_fonts_check_noto_trademark(ttFont):
    ...
    if not trademarks:
        bad = True
        yield FAIL,\
                  Message("no-trademark",
                          "The font contained no trademark entry"
                         )
    for trademark in trademarks:
        if not re.match(TRADEMARK, trademark):
            bad = True
            yield FAIL,\
                  Message("bad-trademark",
...
def com_google_fonts_check_cmap_format_12(ttFont, config):
    ...
    if not cmap4:
        yield FAIL,\
                  Message("no-cmap-4",
                    "The font did not contain a format 4 cmap table"
                  )
        return
...
def com_google_fonts_check_htmx_whitespace_advances(ttFont, config, glyph_metrics_stats):
