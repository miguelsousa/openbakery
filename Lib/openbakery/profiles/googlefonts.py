...
def com_google_fonts_check_usweightclass(ttFont, expected_font_names):
    ...
    from openbakery.profiles.shared_conditions import (
        is_ttf,
        is_cff,
        is_variable_font
    )
...
def com_google_fonts_check_slant_direction(ttFont, uharfbuzz_blob):
    ...
    from openbakery.utils import (axis,
                                  PointsPen)
    import uharfbuzz as hb
...
def com_google_fonts_check_vertical_metrics(ttFont):
    ...
    for k in expected_metrics:
        if font_metrics[k] != expected_metrics[k]:
            ...
            yield FAIL,\
                  Message(f'bad-{k}',
                          f'{k} is "{font_metrics[k]}" it should'
                          f' be {expected_metrics[k]}')
...
def com_google_fonts_check_varfont_grade_reflow(ttFont, config):
    ...
    from openbakery.utils import (all_kerning,
                                  pretty_print_list)
...
def com_google_fonts_check_metadata_designer_profiles(family_metadata, config):
    ...
    from openbakery.utils import get_DesignerInfoProto_Message
    import requests
