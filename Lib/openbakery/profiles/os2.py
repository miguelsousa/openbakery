...
from openbakery.fonts_profile import profile_factory # NOQA pylint: disable=unused-import
...
@check(
    ...
    rationale = """
        When a font project's Vendor ID is specified explicitly on OpenBakery's
...
def com_thetypefounders_check_vendor_id(config, ttFont):
    ...
    if "vendor_id" not in config:
        yield SKIP, ("Add the `vendor_id` key to a `openbakery.yaml` file"
